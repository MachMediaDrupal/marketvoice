...
$("#searchClicked input[type=text]").focusout(function() {
  $("#searchClicked").hide();
   $("#clickSearch").show();
});
...
$('#edit-search-block-form--2').focus();
 ...
 if($.cookie("logincookie") == 4   && (Drupal.settings.currentUser ) <= 0 )
    { 
     $('#register_first').click(function() { // 'this' is not a jQuery object, so it will use

    // the default click() function
 $('.modal-body').html('<div class="popup-logo"><img src="'+ url + '/sites/all/themes/multipurpose/images/popuphead.png"/></div><p>We hope you are enjoying the first 3 of your 5 complimentary articles this month. Do you want free and unlimited access to all of MarketVoice’s in-depth articles, data and opinion? Registering is quick and easy.<a href="?q=user/register"> REGISTER HERE </a></p><p> Already Registered?  <a href="?q=user">Login here</a></p> ');
        this.click();
        
    }).click(); 
  }
  if($.cookie("logincookie") > 5  && (Drupal.settings.currentUser ) <= 0 ) {  
...
$(".modal-header").addClass("modal-header");;
       $(".modal-body").html(
'<div class="popup-logo"><img src="'+ url + '/sites/all/themes/multipurpose/images/popuphead.png"/></div> <p>We hope you have enjoyed your 5 complimentary articles this month. Do you want free and unlimited access to all of the great content on MarketVoice?  Registering is quick and easy.<a href="?q=user/register"> REGISTER HERE </a></p><p> Already Registered?  <a href="?q=user">Login here</a></p> ');
        $(".close").hide();
        $('#subscribeBtn').click(function() { // 'this' is not a jQuery object, so it will use
         						// the default click() function
            this.click();
...
/*
$(".widget-author-details").each(function(){
                   var imgStatus = $(this).find("img").length;
                  if(imgStatus > 0){
                   var posImg = $(this).find('figure').css("float");
                     if(posImg== "left"){
                       $(this).find(".author-descpin").addClass("margined-left");
                     }
                    else{
                      $(this).find(".author-descpin").addClass("margined-right");
                    }
                  }
                });*/
              ...
              $("#owl-demo").owlCarousel({
                ...
                function afterAction(){
                    ...
                    updateResult(".dragDirection", this.owl.dragDirection);
                     ...
                     $(document).ready(function() {
if( $('a').hasClass('image-popup-no-margins')){
     $('.image-popup-no-margins').magnificPopup({
             ...
             $('.owl-wrapper .owl-item').click(function(){
                   $('.currentItem .result').html($(this).index()+1);
