...
<div class="widget-head-title"> 
                              <span class="head-bg"><img src="<?php echo $imagePath?>/images/insight87.png"  class=""/></span>
                               <div class="widget-head-contents clearfix">
                                   <h2>INSIGHT</h2>
                                   <p class="sub-head">A Message from Walt Lukken, President and CEO of FIA and FIA Global</p>
                               </div>
                               
<?php $author = '';
        $field_name = 'field_insight_add_author';
...
foreach($query as $data)  $publishDate = $data->field_date_value; 
 ...
 if($author) {?><?php echo "By ".  rtrim($author, ", ") .  " | "; ?> <?php }?> <?php echo gmdate("F d, Y", $publishDate);?></span> 
			       ...
			       <?php /*Added sidebar*/?>
                                   ...
                                   <?php }?></div></div>
                             <?php }?>
                             <?php $field_name = 'field_add_content_quote_right';
                             $field= field_get_items('node', $node, $field_name); 
                             if( $field[0]['value']) { print $field[0]['value'];}?>
							 
							<?php $field_name = 'field_insight_download_pdf';
								$field= field_get_items('node', $node, $field_name); 
								if( $field[0]['value']) {?>
